...
class Exp_Short_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        path = os.path.join(self.args.checkpoints, setting + '-' + self.args.model_comment)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(self.train_loader)):
                ...
                outputs, res = self.model(batch_x, None, dec_inp, None)
                ...
                loss = loss_value + self.args.sim_coef*res['simlarity_loss'] + self.args.orthogonal_coef * res['orthogonal_loss']
            ...
            vali_loss = self.vali(self.train_loader, self.vali_loader, criterion)
            test_loss = vali_loss
    ...
    def vali(self, train_loader, vali_loader, criterion):
        ...
        with torch.no_grad():
            ...
            dec_inp = torch.cat([x[:, -self.args.label_len:, :], dec_inp], dim=1)
            # encoder - decoder
            outputs = torch.zeros((B, self.args.pred_len, C)).float().to(self.device)
            id_list = np.arange(0, B, self.args.eval_batch_size)  # validation set size
            id_list = np.append(id_list, B)
            for i in range(len(id_list) - 1):
                outputs[id_list[i]:id_list[i + 1], :, :], _ = self.model(x[id_list[i]:id_list[i + 1]], None,
    ...
    def test(self, setting, test=0):
        x, _ = self.train_loader.dataset.last_insample_window()
        y = self.test_loader.dataset.timeseries
        x = torch.tensor(x, dtype=torch.float32).to(self.device)
        x = x.unsqueeze(-1)
        ...
        forecasts_df.index = self.test_loader.dataset.ids[:preds.shape[0]]
